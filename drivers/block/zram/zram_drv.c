				...
				unsigned long pages, bool trigger)
{
	...
	unsigned long writtenback = atomic64_read(&zram->stats.bd_count) -
	...
	while (!kthread_should_stop()) {
		...
		list_for_each_entry_safe(zram_entry, n, &zram->list, lru_list) {
			...
			if (!zram_try_mark_page(zram, index))
				continue;
			if (!zram_writeback_index(zram, index, page))
				count++;
			if (!zram_should_writeback(zram, count, false))
				break;
		}
		zram->wbd_running = false;
		pr_info("%s writtenback %lu pages", __func__, count);
	}
	__free_page(page);
