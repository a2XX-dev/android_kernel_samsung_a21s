				   ...
				   struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (data->proximity_ops == NULL) {
		return -EINVAL;
	}
	if (data->proximity_ops->set_proximity_calibration == NULL) {
		int result = 0;
...
void select_prox_ops(struct ssp_data *data)
{
	...
	count++;
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3X6X)
	count++;
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_GP2AP110S)
	count++;
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3A5X)
	count++;
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3328)
	count++;
#endif
...
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3X6X)
	prox_ops_ary[i++] = get_proximity_stk3x6x_function_pointer(data);
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_GP2AP110S)
	prox_ops_ary[i++] = get_proximity_gp2ap110s_function_pointer(data);
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3A5X)
	prox_ops_ary[i++] = get_proximity_stk3a5x_function_pointer(data);
#endif
#if defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3328)
	prox_ops_ary[i++] = get_proximity_stk3328_function_pointer(data);
#endif
...
void initialize_prox_factorytest(struct ssp_data *data)
{
	...
	data->proximity_ops = get_proximity_stk3x6x_function_pointer(data);
	...
	data->proximity_ops = get_proximity_stk3a5x_function_pointer(data);
#elif defined(CONFIG_SENSORS_SSP_PROXIMITY_STK3328)
	data->proximity_ops = get_proximity_stk3328_function_pointer(data);
#endif
